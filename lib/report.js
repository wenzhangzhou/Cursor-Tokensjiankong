...
function pickComposer(event, composers) {
  const conversationId = String((event && event.conversationId) || "");
  if (conversationId) {
    const exact = composers.find((item) => item.composerId === conversationId);
    if (exact) return exact;
  }
  const ts = eventTime(event);
  if (ts == null) return null;
  const hits = composers.filter((item) => {
    if (!item.createdAt || !item.lastUpdatedAt) return false;
    return ts >= item.createdAt - 60 * 1000 && ts <= item.lastUpdatedAt + 2 * 60 * 1000;
  });
  if (!hits.length) return null;
  hits.sort((a, b) => {
    const widthA = a.lastUpdatedAt - a.createdAt;
    const widthB = b.lastUpdatedAt - b.createdAt;
...
function buildSummary(dataDir) {
  ...
  for (const event of Array.isArray(events) ? events : []) {
    const ts = eventTime(event);
    if (ts == null) continue;
    if (ts >= todayStart) addTotals(periods.today, event);
    if (ts >= thisWeek) addTotals(periods.week, event);
    if (ts >= thisMonth) addTotals(periods.month, event);
    ...
    const composer = pickComposer(event, composers);
    const taskId = composer ? composer.composerId : "unmatched";
    if (!byTask.has(taskId)) {
      const hooked = composer ? titles.get(composer.composerId) : "";
      byTask.set(taskId, {
        id: taskId,
        title: hooked || displayTitle(composer),
        untitled: !(hooked || (composer && composer.name) || (composer && composer.isSubagent && composer.subagentTypeName)),
        isSubagent: Boolean(composer && composer.isSubagent),
        workspace: (composer && composer.workspace) || "",
        lastAt: ts,
        totals: blankTotals(),
      });
    }
